...
<form method="post" class="container mt-4 p-5 border border-1 rounded" id="form" x-data x-effect="setNotification()">
  <div class=""></div>
  <h5 class="text-center mb-5">Form Edit Jenis Paket Cucian</h5>
  <div class="mb-3">
...
<script>
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.store('global', {
      form: {
        'jenis_paket': `<?= $dataJenisPaket['jenis_paket'] ?>`
      },
      validasi: {},
      state: {
        loading: false,
  ...
  function setNotification() {
    ...
    if (status == 'sukses') {
      ...
      Toast.fire({
        icon: 'success',
        title: 'jenis paket cucian berhasil diubah'
      })
    }
    ...
    if (status == 'gagal') {
      Toast.fire({
        icon: 'error',
        title: 'jenis paket cucian gagal diubah'
      })
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.put('<?= base_url() ?>api/JenisPaket/<?= $dataJenisPaket['id_jenis_paket'] ?>', data);
