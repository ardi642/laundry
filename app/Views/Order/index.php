...
<div class="container mt-5 p-5 border border-1 rounded" x-data>
  <h5 class="text-center mb-4">Tabel Order Cucian</h5>
  <div class="text-center mb-5">
    <a class="btn btn-success" href="/Order/tambah" role="button">
      Tambah Order Cucian
    </a>
  </div>
  <div class="row mb-4">
  ...
  let tabel = $('#tabel').DataTable({
    ...
    columns: [{
      ...
      },
      {
        ...
        render: function(data, type, row, meta) {
          return `
            <a href="Order/detail/${row.id_order}" class="btn btn-sm btn-success">Detail</a>
            ${row.status != 'sudah diambil' ? 
              `<a href="Order/edit/${row.id_order}" class="btn btn-sm btn-warning">Edit</a>`
            : ""}
