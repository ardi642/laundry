...
<form method="post" class="container mt-4 p-5 border border-1 rounded" id="form" x-data x-effect="setNotification()" x-init="$nextTick(() => {aturKeterangan()})">
  <div x-init="$watch('$store.global.paketSelected.data', aturKeterangan)"></div>
  <div x-init="$watch('$store.global.form', aturKeterangan)"></div>
  <h5 class="text-center mb-5">Form Edit Order Cucian Pelanggan</h5>
  <div class="mb-3">
...
<script>
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.store('global', {
      form: {
        nama_pelanggan: `<?= $dataOrder['nama_pelanggan'] ?>`,
        no_telepon: `<?= $dataOrder['no_telepon'] ?>`,
        jumlah: <?= $dataOrder['jumlah'] ?>,
        status: `<?= $dataOrder['status'] ?>`,
        alamat: `<?= $dataOrder['alamat'] ?>`,
        uang_pelanggan: <?= $dataOrder['uang_pelanggan'] ?>,
      ...
      paketSelected: {
        id_paket: <?= $dataOrder['id_paket'] ?>,
        nama_paket: `<?= $dataOrder['nama_paket'] ?>`,
        data: {
          tarif_satuan: <?= $dataOrder['tarif_satuan_tertentu'] ?>,
          satuan: `<?= $dataOrder['satuan_tertentu'] ?>`
        }
      },
      keterangan: {
        tarif: null,
  ...
  function setNotification() {
    ...
    if (status == 'sukses') {
      Toast.fire({
        icon: 'success',
        title: 'data order cucian berhasil diubah'
      })
    }
    ...
    if (status == 'gagal') {
      Toast.fire({
        icon: 'error',
        title: 'data order cucian gagal diubah'
      })
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.put('<?= base_url() ?>api/Order/<?= $dataOrder['id_order'] ?>', data);
