...
<form method="post" class="container mt-4 p-5 border border-1 rounded" id="form" x-data x-effect="setNotification()">
  <div class=""></div>
  <h5 class="text-center mb-5">Form Edit Paket Cucian</h5>
  <div class="mb-3">
...
<script>
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.store('global', {
      jenisPakets: [],
      form: {
        id_jenis_paket: <?= $dataPaket['id_jenis_paket'] ?>,
        nama_paket: `<?= $dataPaket['nama_paket'] ?>`,
        waktu_kerja_jam: <?= $dataPaket['waktu_kerja_jam'] ?>,
        waktu_kerja_hari: <?= $dataPaket['waktu_kerja_hari'] ?>,
        satuan: `<?= $dataPaket['satuan'] ?>`,
        tarif_satuan: <?= $dataPaket['tarif_satuan'] ?>,
        minimal_satuan: <?= $dataPaket['minimal_satuan'] ?>,
        keterangan: `<?= $dataPaket['keterangan'] ?>`
      },
      validasi: {},
      state: {
        loading: false,
  ...
  async function loadSelectJenisPaket() {
    ...
    try {
      ...
      this.$nextTick(() => {
        storeGlobal.form.id_jenis_paket = <?= $dataPaket['id_jenis_paket'] ?>;
      })
  ...
  function setNotification() {
    ...
    if (status == 'sukses') {
      Toast.fire({
        icon: 'success',
        title: 'paket cucian berhasil diubah'
      })
    }
    ...
    if (status == 'gagal') {
      Toast.fire({
        icon: 'error',
        title: 'paket cucian gagal diubah'
      })
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.put('<?= base_url() ?>api/Paket/<?= $dataPaket['id_paket'] ?>', data);
