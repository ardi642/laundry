...
<script>
  ...
  let tabel = $('#tabel').DataTable({
    ...
    columns: [{
      ...
      },
      {
        ...
        render: function(data, type, row, meta) {
          return `
            <a href="<?= base_url() ?>karyawan/edit/${row.id_karyawan}" class="btn btn-sm btn-warning">Edit</a>
  ...
  $('#tabel').on('click', '.tombol-hapus', async function(e) {
    ...
    try {
      const response = await axios.delete(`<?= base_url() ?>api/Karyawan/${rowData.id_karyawan}`);
      status = 'sukses';
    } catch (error) {
      console.log(error);
